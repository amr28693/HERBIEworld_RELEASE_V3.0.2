...
class TorusBrain:
    ...
    - Coherence: concentration of ||^2 (focus/attention)
    ...
    def inject_at_phase(self, phase: float, strength: float):
        """
        Inject a signal at a specific phase (for global field coupling).
        
        This entrains the torus to external rhythms like the global
        Schumann-like resonance field, creating inter-creature coherence.
        
        Args:
            phase: Phase angle in radians
            strength: Injection strength
        """
        # Create a gentle injection that biases phase without overwhelming
        injection = strength * np.exp(1j * phase)
        
        # Apply to all positions with Gaussian weighting from current state
        current_density = np.abs(self.psi) ** 2
        if np.sum(current_density) > 0.01:
            weights = current_density / np.sum(current_density)
        else:
            weights = np.ones(N_torus) / N_torus
        
        self.psi += injection * weights * 0.1
    ...
    def evolve(self, audio_amp: float, hunger: float, dream_depth: float):
        ...
        # Clamp intensity before nonlinear term to prevent blowup
        I_clamped = np.clip(np.abs(self.psi)**2, 0, 5.0)
        self.psi *= np.exp(1j * self.g * dt * I_clamped)
        
        # Dissipation
        self.psi *= 0.996
        
        # NaN guard - reset if field has blown up
        if np.any(np.isnan(self.psi)):
            random_phase = np.random.uniform(0, 2*np.pi, N_torus)
            random_amp = 0.4 + 0.2 * np.random.rand(N_torus)
            self.psi = random_amp * np.exp(1j * random_phase)
...
class GhostTorus:
    ...
    def to_dict(self) -> dict:
        """Serialize ghost for persistence."""
        return {
            'psi_real': self.psi.real.tolist(),
            'psi_imag': self.psi.imag.tolist(),
            'pos': self.pos.tolist(),
            'vel': self.vel.tolist(),
            'name': self.name,
            'initial_circulation': self.initial_circulation,
    ...
    @classmethod
    def from_dict(cls, d: dict) -> 'GhostTorus':
        """Deserialize ghost from persistence."""
        psi = np.array(d['psi_real']) + 1j * np.array(d['psi_imag'])
        pos = np.array(d['pos'])
        ghost = cls(
            psi=psi,
            pos=pos,
            name=d.get('name'),
            circulation=d.get('initial_circulation', 0.0),
            energy=d.get('initial_energy', 0.0),
            g=d.get('g', -0.5)
        )
        ghost.vel = np.array(d.get('vel', [0.0, 0.0]))
        ghost.age = d.get('age', 0)
        ghost.alive = d.get('alive', True)
        return ghost
    ...
    def update(self, terrain: 'Terrain', other_ghosts: list = None) -> bool:
        ...
        Movement emerges purely from wavefunction dynamics.
        """
        if not self.alive:
            return False
        ...
        # Pure emergent: movement comes only from phase winding in wavefunction
        phase = np.angle(self.psi)
        dphase = np.diff(np.unwrap(phase))
        circulation = np.mean(dphase)
        
        # Speed proportional to circulation - no artificial minimum
        # Some ghosts will sit still if their wavefunction has no net winding
        speed = np.abs(circulation) * 0.12
        angle = self.age * circulation * 0.1
        
        self.vel = self.vel * 0.95 + np.array([np.cos(angle), np.sin(angle)]) * speed * 0.05
        self.pos += self.vel
...
class GhostField:
    ...
    def __init__(self):
        ...
        self.named_ghosts_spawned = []  # Track named spirits
        self.longest_lived_age = 0
        self.longest_lived_name = None
    ...
    def spawn_ghost(self, brain: 'TorusBrain', pos: np.ndarray, 
        ...
        if name:
            self.named_ghosts_spawned.append(name)
    ...
    def update(self, terrain: 'Terrain'):
        ...
        for ghost in dispersing:
            ...
            # Track longest-lived
            if ghost.age > self.longest_lived_age:
                self.longest_lived_age = ghost.age
                self.longest_lived_name = ghost.name
            
            if cause == "singularity":
                self.total_singularities += 1
                if ghost.name:
                ...
                if not np.any(np.isnan(ghost.pos)):
                    ...
                    if ghost.name:
                        print(f"[Ghost]  {ghost.name} dispersed into the earth")
            
            self.total_dispersed += 1
        
        self.ghosts = [g for g in self.ghosts if g.alive]
        
        # Check for ghost congregations (rare event logging)
        if len(self.ghosts) >= 2:
            centers = self.get_congregation_centers(min_ghosts=2)
            for center in centers:
                if center['total_coherence'] > 1.5:  # Strong congregation
                    names = center['names']
                    if names and not hasattr(self, '_last_congregation') or self._last_congregation != tuple(names):
                        self._last_congregation = tuple(names)
                        name_str = " and ".join(names[:3])
                        if len(names) > 3:
                            name_str += f" and {len(names)-3} others"
                        print(f"[Ghost]  Spirits gathering... {name_str}")
    ...
    def get_statistics(self) -> dict:
        """Get ghost field statistics for end-of-simulation report."""
        alive_ghosts = [g for g in self.ghosts if g.alive]
        
        # Current state
        current_count = len(alive_ghosts)
        avg_coherence = np.mean([g._compute_coherence() for g in alive_ghosts]) if alive_ghosts else 0
        avg_age = np.mean([g.age for g in alive_ghosts]) if alive_ghosts else 0
        
        # Named spirits currently wandering
        current_named = [g.name for g in alive_ghosts if g.name]
        
        return {
            'total_spawned': self.total_spawned,
            'total_dispersed': self.total_dispersed,
            'total_singularities': self.total_singularities,
            'currently_wandering': current_count,
            'current_named': current_named,
            'avg_coherence': avg_coherence,
            'avg_age': avg_age,
            'longest_lived_age': self.longest_lived_age,
            'longest_lived_name': self.longest_lived_name,
            'named_spawned': self.named_ghosts_spawned.copy()
        }
    
    def to_dict(self) -> dict:
        """Serialize ghost field for persistence."""
        return {
            'ghosts': [g.to_dict() for g in self.ghosts if g.alive],
            'total_spawned': self.total_spawned,
            'total_dispersed': self.total_dispersed,
            'total_singularities': self.total_singularities,
            'longest_lived_age': self.longest_lived_age,
            'longest_lived_name': self.longest_lived_name,
            'named_ghosts_spawned': self.named_ghosts_spawned.copy(),
        }
    
    @classmethod
    def from_dict(cls, d: dict) -> 'GhostField':
        """Deserialize ghost field from persistence."""
        field = cls()
        field.total_spawned = d.get('total_spawned', 0)
        field.total_dispersed = d.get('total_dispersed', 0)
        field.total_singularities = d.get('total_singularities', 0)
        field.longest_lived_age = d.get('longest_lived_age', 0)
        field.longest_lived_name = d.get('longest_lived_name', None)
        field.named_ghosts_spawned = d.get('named_ghosts_spawned', [])
        
        # Restore ghosts
        for ghost_data in d.get('ghosts', []):
            try:
                ghost = GhostTorus.from_dict(ghost_data)
                field.ghosts.append(ghost)
            except Exception as e:
                print(f"[Ghost] Failed to restore ghost: {e}")
        
        if field.ghosts:
            print(f"[Ghost] Restored {len(field.ghosts)} wandering spirits")
