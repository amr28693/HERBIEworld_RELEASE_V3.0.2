...
class GripperLimb:
    ...
    def evolve(self, dt: float = 0.015):
        ...
        # === ARM SWING PHYSICS ===
        # Wave momentum in the arm creates angular acceleration
        # This is pure physics: wave energy -> mechanical motion
        # No behavior decisions here - just F=ma for rotation
        
        # Net wave momentum (asymmetric = torque)
        wave_momentum = np.sum(self.u * self.x_grip)  # Weighted by position along arm
        
        # Angular acceleration proportional to wave momentum
        angular_accel = wave_momentum * 0.02
        
        # Spring force back toward base angle (passive mechanics)
        spring_force = (self.base_angle - self.angle) * 0.1
        
        # Damping (air resistance, tissue friction)
        damping_force = -self.angular_velocity * 0.3
        
        # Update angular velocity and angle
        self.angular_velocity += (angular_accel + spring_force + damping_force) * dt
        self.angular_velocity = np.clip(self.angular_velocity, -3.0, 3.0)
        self.angle += self.angular_velocity * dt
        
        # Angle limits (physical joint constraints)
        if self.side == 'left':
            self.angle = np.clip(self.angle, 0.0, np.pi)
        else:
            self.angle = np.clip(self.angle, -np.pi, 0.0)
    ...
    def use_as_tool(self, body_pos: np.ndarray, target_pos: np.ndarray,
        ...
        """
        Use held object as a tool. Returns damage dealt.
        
        PHYSICS: Damage is real - drains body energy and increases hunger.
        Enough accumulated damage can kill, just like predation.
        """
        if self.held_object is None:
            return 0.0
        ...
        # Base damage from tool properties
        base_damage = grip_props.tool_damage
        
        # Swing factor - faster swing = more damage (kinetic energy)
        swing_factor = 1.0 + abs(self.angular_velocity) * 0.8
        
        # Timing factor - better neural coordination = more effective
        timing_factor = 1.0 + self.tip_activation * 0.3
        
        damage = base_damage * swing_factor * timing_factor
        
        if target_creature is not None and damage > 0:
            # Apply REAL damage like predator attacks
            # 1. Energy drain (direct trauma)
            if hasattr(target_creature, 'body'):
                target_creature.body.energy *= (1.0 - damage * 0.15)
            
            # 2. Hunger increase (metabolic stress from injury)
            if hasattr(target_creature, 'metabolism'):
                target_creature.metabolism.hunger = min(1.0,
                    target_creature.metabolism.hunger + damage * 0.3)
            
            # 3. Pain signal
            if hasattr(target_creature, 'afferent') and 'env_pain' in target_creature.afferent:
                target_creature.afferent['env_pain'].nucleate(damage * 2.0, 0.0)
            
            # 4. Check for death (same conditions as predation)
            if hasattr(target_creature, 'metabolism') and hasattr(target_creature, 'body'):
                if target_creature.metabolism.hunger >= 0.99 or target_creature.body.energy < 5:
                    target_creature.alive = False
                    target_creature.metabolism.is_dead = True
                    target_creature.metabolism.cause_of_death = "trauma"
...
class HerbieHands:
    ...
    def check_collisions(self, body_pos: np.ndarray, nearby_creatures: List) -> List[dict]:
        """
        PHYSICS ONLY: Check if arm tips collide with nearby creatures.
        
        This is pure collision detection - like checking if a ball hits a wall.
        No behavior logic. No decisions. Just physics.
        
        Returns list of collision events (for logging/observation).
        """
        collisions = []
        
        # Collision radius (how close tip must be to count as contact)
        CONTACT_RADIUS = 1.5
        
        for hand in [self.left, self.right]:
            tip_pos = hand.get_tip_position(body_pos)
            
            for creature in nearby_creatures:
                if not creature.alive:
                    continue
                
                # Distance from tip to creature center
                dist = np.linalg.norm(tip_pos - creature.pos)
                
                # Creature body radius (approximate)
                creature_radius = getattr(creature, 'size', 1.0) * 0.5
                
                # Check contact
                if dist < CONTACT_RADIUS + creature_radius:
                    # Contact! Apply physics.
                    # Damage depends on: what's held, arm velocity, tip activation
                    
                    damage = hand.use_as_tool(body_pos, creature.pos, creature)
                    
                    if damage > 0:
                        collisions.append({
                            'hand': hand.side,
                            'target_id': creature.creature_id,
                            'damage': damage,
                            'tool_held': hand.held_object is not None,
                            'angular_velocity': hand.angular_velocity,
                        })
