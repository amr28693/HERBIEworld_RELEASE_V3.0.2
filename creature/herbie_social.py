...
HERBIE_COURTSHIP_DISTANCE = 10.0      # Increased from 8.0 - easier to stay close
HERBIE_COURTSHIP_DURATION = 80        # Reduced from 150 - faster courtship
HERBIE_MATING_HUNGER_MAX = 0.6        # Increased from 0.4 - can mate when moderately hungry
HERBIE_MATING_MIN_AGE = 0             # No artificial age gate - maturity is physical not temporal

# Pregnancy
HERBIE_GESTATION_DURATION = 350       # Reduced from 400 - faster pregnancies
HERBIE_PREGNANCY_SPEED_PENALTY = 0.7  # Increased from 0.6 - less mobility penalty
HERBIE_PREGNANCY_HUNGER_MULT = 1.3    # Reduced from 1.5 - less metabolic burden
...
# Resonance
HERBIE_RESONANCE_THRESHOLD = 0.35         # Lowered from 0.5 - fewer failed resonances
HERBIE_RESONANCE_BOND_THRESHOLD = 0.55    # Lowered from 0.7 - easier to bond
...
class DevelopmentContext:
    ...
    Allows tracking of emergent epigenetics.
    """
    # Mother's state at conception
    mother_energy: float = 100.0
    mother_stress: float = 0.0  # skeleton RMS average during pregnancy
    mother_hunger: float = 0.0
    mother_dream_depth: float = 0.0  # How much dreaming during gestation
    
    # Environmental
    audio_exposure: float = 0.0  # Average audio amplitude during development
    season_at_birth: str = "spring"
    time_of_day_at_birth: float = 0.5
    
    # Social
    n_nearby_herbies: int = 0  # Crowding/social exposure
    predator_exposure: float = 0.0
    
    def to_dict(self) -> dict:
        return {
            'mother_energy': self.mother_energy,
            'mother_stress': self.mother_stress,
            'mother_hunger': self.mother_hunger,
            'mother_dream_depth': self.mother_dream_depth,
            'audio_exposure': self.audio_exposure,
            'season_at_birth': self.season_at_birth,
            'time_of_day_at_birth': self.time_of_day_at_birth,
            'n_nearby_herbies': self.n_nearby_herbies,
            'predator_exposure': self.predator_exposure,
        }
    
    @classmethod
    def from_dict(cls, d: dict) -> 'DevelopmentContext':
        return cls(**d)


@dataclass
class HeritableState:
    """
    Pure tracking of what gets inherited - NO IMPOSED GENETIC STRUCTURE.
    
    This captures the actual wavefunction states and phenotypes of parents
    at conception, allowing post-hoc analysis of inheritance patterns
    without asserting any particular genetic architecture.
    
    The correlations between parent states and offspring outcomes
    are discovered in the data, not hardcoded.
    """
    # Parent wavefunction snapshots at conception (the actual "genetic material")
    parent_a_torus_sample: Optional[np.ndarray] = None  # Flattened |ψ|² from torus
    parent_b_torus_sample: Optional[np.ndarray] = None
    
    # Parent torus attractor basin info (emergent gene-like structures)
    parent_a_basins: Optional[List[dict]] = None  # [{pos, depth, width}, ...]
    parent_b_basins: Optional[List[dict]] = None
    
    # Parent phenotypes at conception (measured, not computed from genes)
    parent_a_phenotype: Optional[Dict[str, float]] = None
    parent_b_phenotype: Optional[Dict[str, float]] = None
    
    # Parent IDs for lineage tracking
    parent_a_id: str = ""
    parent_b_id: str = ""
    
    # Mixing parameters (what actually happened, not what we prescribed)
    torus_mixing_ratio: float = 0.5  # How much of each parent's torus influenced offspring
    
    def to_dict(self) -> dict:
        return {
            'parent_a_torus_sample': self.parent_a_torus_sample.tolist() if self.parent_a_torus_sample is not None else None,
            'parent_b_torus_sample': self.parent_b_torus_sample.tolist() if self.parent_b_torus_sample is not None else None,
            'parent_a_basins': self.parent_a_basins,
            'parent_b_basins': self.parent_b_basins,
            'parent_a_phenotype': self.parent_a_phenotype,
            'parent_b_phenotype': self.parent_b_phenotype,
            'parent_a_id': self.parent_a_id,
            'parent_b_id': self.parent_b_id,
            'torus_mixing_ratio': self.torus_mixing_ratio,
        }
    
    @classmethod
    def from_dict(cls, d: dict) -> 'HeritableState':
        state = cls()
        if d.get('parent_a_torus_sample') is not None:
            state.parent_a_torus_sample = np.array(d['parent_a_torus_sample'])
        if d.get('parent_b_torus_sample') is not None:
            state.parent_b_torus_sample = np.array(d['parent_b_torus_sample'])
        state.parent_a_basins = d.get('parent_a_basins')
        state.parent_b_basins = d.get('parent_b_basins')
        state.parent_a_phenotype = d.get('parent_a_phenotype')
        state.parent_b_phenotype = d.get('parent_b_phenotype')
        state.parent_a_id = d.get('parent_a_id', '')
        state.parent_b_id = d.get('parent_b_id', '')
        state.torus_mixing_ratio = d.get('torus_mixing_ratio', 0.5)
        return state
    
    @classmethod
    def capture_from_parents(cls, parent_a, parent_b) -> 'HeritableState':
        """
        Capture heritable state from two parent creatures.
        
        This samples their actual wavefunction states - no interpretation.
        """
        state = cls()
        
        # Capture torus wavefunction density
        if hasattr(parent_a, 'torus') and parent_a.torus is not None:
            state.parent_a_torus_sample = np.abs(parent_a.torus.psi.flatten())**2
            state.parent_a_basins = cls._extract_basins(parent_a.torus)
        
        if hasattr(parent_b, 'torus') and parent_b.torus is not None:
            state.parent_b_torus_sample = np.abs(parent_b.torus.psi.flatten())**2
            state.parent_b_basins = cls._extract_basins(parent_b.torus)
        
        # Capture parent phenotypes (measured values)
        state.parent_a_phenotype = cls._measure_phenotype(parent_a)
        state.parent_b_phenotype = cls._measure_phenotype(parent_b)
        
        # IDs
        state.parent_a_id = parent_a.creature_id
        state.parent_b_id = parent_b.creature_id
        
        # Mixing ratio is random (what nature does)
        state.torus_mixing_ratio = np.random.random()
        
        return state
    
    @staticmethod
    def _extract_basins(torus) -> List[dict]:
        """Extract attractor basin info from torus wavefunction."""
        if torus is None or torus.psi is None:
            return []
        
        density = np.abs(torus.psi)**2
        ...
        for coord in peak_coords[:10]:  # Limit to top 10
            # coord is shape (1,) for 1D array, (2,) for 2D
            if len(coord) == 1:
                i = coord[0]
                basins.append({
                    'pos': [int(i)],
                    'depth': float(density[i]),
                    'width': 1.0
                })
            else:
                i, j = coord
    ...
    @staticmethod
    def _measure_phenotype(creature) -> Dict[str, float]:
        """Measure actual phenotype values from creature state."""
        pheno = {}
        
        # Physical
        pheno['body_scale'] = getattr(creature, '_genome_scale', 1.0)
        pheno['speed'] = getattr(creature, '_genome_speed_mult', 1.0)
        # Energy is on body field, not metabolism
        if hasattr(creature, 'body') and hasattr(creature.body, 'energy'):
            pheno['energy'] = creature.body.energy
        else:
            pheno['energy'] = 100.0
        
        # Behavioral (from genome if Herbie)
        if hasattr(creature, 'mating_state') and hasattr(creature.mating_state, 'genome'):
            g = creature.mating_state.genome
            pheno['aggression'] = g.aggression
            pheno['sociability'] = g.sociability
            pheno['curiosity'] = g.curiosity
            pheno['audio_sensitivity'] = g.audio_sensitivity
        
        # Neural state
        if hasattr(creature, 'torus') and creature.torus is not None:
            pheno['torus_arousal'] = float(np.std(np.abs(creature.torus.psi)**2))
            pheno['torus_coherence'] = float(np.abs(np.mean(creature.torus.psi)))
...
class HerbieGenome:
    """
    Genetic traits for Herbies.
    
    DESIGN PHILOSOPHY:
    - Phenotype values are inherited with noise, NOT computed from Mendelian alleles
    - The Mendelian system is OPTIONAL and kept only for visualization/education
    - HeritableState tracks the actual wavefunction states for scientific analysis
    - Correlations between inheritance and outcomes are DISCOVERED, not asserted
    """
    # Physical phenotypes (inherited with variation)
    ...
    # Hand dexterity phenotypes
    grip_strength: float = 0.5
    reach_modifier: float = 1.0
    
    # === TRACKING (no imposed structure) ===
    heritable_state: HeritableState = None  # Pure tracking of parent states
    development: DevelopmentContext = None  # Environmental context
    
    # === OPTIONAL: Mendelian layer (for visualization only) ===
    mendelian: 'Genome' = None  # Can be None - not required for function
    
    def __post_init__(self):
        if self.development is None:
            self.development = DevelopmentContext()
        if self.heritable_state is None:
            self.heritable_state = HeritableState()
        # Note: mendelian is NOT auto-created - it's optional now
    ...
    def random(cls) -> 'HerbieGenome':
        ...
        genome.development = DevelopmentContext()
        genome.heritable_state = HeritableState()
        
        # Optionally create Mendelian genome for visualization
        # (disabled by default to avoid imposing structure)
        # from .genetics import create_random_genome
        # genome.mendelian = create_random_genome()
        
        return genome
    
    @classmethod
    def from_parents(cls, parent_a: 'HerbieGenome', parent_b: 'HerbieGenome',
                     development_context: DevelopmentContext = None,
                     parent_a_creature=None, parent_b_creature=None) -> 'HerbieGenome':
        """
        Create offspring genome from two parents.
        
        Inheritance is simple mixing with noise - NO Mendelian rules.
        The actual inheritance patterns emerge from the simulation dynamics.
        """
        child = cls()
        
        # Inherit phenotypes with mixing and noise
        for field_name in ['body_scale', 'speed_modifier', 'energy_efficiency',
            ...
            # Random mixing ratio per trait
            mix = np.random.random()
            base_val = mix * val_a + (1 - mix) * val_b
            
            # Add noise (mutation equivalent, but emergent)
            noise = np.random.normal(0, 0.05)
            setattr(child, field_name, base_val + noise)
        
        # Clamp to valid ranges
        child._clamp_phenotypes()
        
        # Store development context
        child.development = development_context or DevelopmentContext()
        
        # Capture heritable state from parent creatures if available
        if parent_a_creature is not None and parent_b_creature is not None:
            child.heritable_state = HeritableState.capture_from_parents(
                parent_a_creature, parent_b_creature
            )
        else:
            child.heritable_state = HeritableState()
            child.heritable_state.parent_a_phenotype = parent_a.get_full_phenotype()
            child.heritable_state.parent_b_phenotype = parent_b.get_full_phenotype()
        
        # Mendelian is NOT computed - kept optional
        # child.mendelian = None
        
        return child
    
    def _clamp_phenotypes(self):
        """Ensure all phenotypes are in valid ranges."""
        self.body_scale = np.clip(self.body_scale, 0.7, 1.4)
        self.speed_modifier = np.clip(self.speed_modifier, 0.6, 1.5)
        self.energy_efficiency = np.clip(self.energy_efficiency, 0.7, 1.4)
        self.aggression = np.clip(self.aggression, 0.1, 0.9)
        self.curiosity = np.clip(self.curiosity, 0.1, 0.9)
        self.sociability = np.clip(self.sociability, 0.1, 0.9)
        self.torus_g_modifier = np.clip(self.torus_g_modifier, -0.5, 0.5)
        self.audio_sensitivity = np.clip(self.audio_sensitivity, 0.8, 2.5)
        self.grip_strength = np.clip(self.grip_strength, 0.2, 0.8)
        self.reach_modifier = np.clip(self.reach_modifier, 0.7, 1.3)
    
    def get_genotype_string(self, gene_name: str) -> str:
        """Get genotype as string (only if Mendelian genome exists)."""
        if self.mendelian is None:
            return "??"
        return self.mendelian.get_genotype_string(gene_name)
    
    def get_full_genotype(self) -> dict:
        """Get all genotypes as strings (only if Mendelian genome exists)."""
        if self.mendelian is None:
            return {}
        return self.mendelian.get_full_genotype()
    
    def get_full_phenotype(self) -> dict:
    ...
    def mix_and_mutate(cls, genome_a: 'HerbieGenome', genome_b: 'HerbieGenome',
                       ...
                       parent_a_creature=None, parent_b_creature=None) -> 'HerbieGenome':
        """
        Sexual reproduction - simple mixing with noise.
        
        Kept for backwards compatibility.
        """
        return cls.from_parents(
            genome_a, genome_b,
            development_context=development_context,
            parent_a_creature=parent_a_creature,
            parent_b_creature=parent_b_creature
        )
    ...
    def to_dict(self) -> dict:
        ...
        # Save heritable state
        if self.heritable_state is not None:
            result['heritable_state'] = self.heritable_state.to_dict()
        
        # Save development context
        if self.development is not None:
            result['development'] = self.development.to_dict()
        
        # Save Mendelian genotype if present (optional)
        if self.mendelian is not None:
            result['mendelian'] = self.mendelian.to_dict()
    ...
    def from_dict(cls, d: dict) -> 'HerbieGenome':
        genome = cls(
            body_scale=d.get('body_scale', 1.0),
            speed_modifier=d.get('speed_modifier', 1.0),
            energy_efficiency=d.get('energy_efficiency', 1.0),
            aggression=d.get('aggression', 0.5),
            curiosity=d.get('curiosity', 0.5),
            sociability=d.get('sociability', 0.5),
            torus_g_modifier=d.get('torus_g_modifier', 0.0),
            audio_sensitivity=d.get('audio_sensitivity', 1.5),
            grip_strength=d.get('grip_strength', 0.5),
            reach_modifier=d.get('reach_modifier', 1.0),
        )
        
        # Restore heritable state
        if 'heritable_state' in d:
            genome.heritable_state = HeritableState.from_dict(d['heritable_state'])
        
        # Restore development context
        if 'development' in d:
            genome.development = DevelopmentContext.from_dict(d['development'])
        
        # Restore Mendelian genotype if present (optional)
        if 'mendelian' in d:
            from .genetics import Genome
            genome.mendelian = Genome.from_dict(d['mendelian'])
...
class HerbieMatingState:
    ...
    mating_cooldown: int = 300  # Reduced from 500 - can attempt mating more often
