...
class WorldHistory:
    ...
    def generate_report(self, final_step: int, final_counts: dict, ghost_stats: dict = None) -> str:
        ...
        # Ghost/Spirit statistics
        if ghost_stats:
            lines.append("-" * 70)
            lines.append("  ✦ THE SPIRIT REALM ✦")
            lines.append("-" * 70)
            lines.append("")
            lines.append(f"  Total spirits spawned: {ghost_stats.get('total_spawned', 0)}")
            lines.append(f"  Total dispersed: {ghost_stats.get('total_dispersed', 0)}")
            lines.append(f"  Singularity collapses: {ghost_stats.get('total_singularities', 0)}")
            lines.append(f"  Currently wandering: {ghost_stats.get('currently_wandering', 0)}")
            lines.append("")
            
            # Longest-lived ghost
            if ghost_stats.get('longest_lived_age', 0) > 0:
                name = ghost_stats.get('longest_lived_name') or 'unnamed spirit'
                age = ghost_stats.get('longest_lived_age', 0)
                lines.append(f"  Longest persisting spirit: {name} ({age:,} steps)")
            
            # Average coherence
            avg_coh = ghost_stats.get('avg_coherence', 0)
            if avg_coh > 0:
                lines.append(f"  Average coherence of active spirits: {avg_coh:.2f}")
            
            # Named spirits that were spawned
            named = ghost_stats.get('named_spawned', [])
            if named:
                lines.append("")
                lines.append(f"  Named spirits who walked the other side ({len(named)}):")
                for name in named[:20]:
                    lines.append(f"    ✦ {name}")
                if len(named) > 20:
                    lines.append(f"    ... and {len(named) - 20} more")
            
            # Currently wandering named
            current = ghost_stats.get('current_named', [])
            if current:
                lines.append("")
                lines.append(f"  Still wandering at simulation end:")
                for name in current:
                    lines.append(f"    ✦ {name}")
            lines.append("")
    ...
    def save_report(self, final_step: int, final_counts: dict, ghost_stats: dict = None, filepath: str = None):
        """Save the report to a file."""
        if filepath is None:
            filepath = os.path.join(DATA_DIR, 'world_history.txt')
        
        report = self.generate_report(final_step, final_counts, ghost_stats)
