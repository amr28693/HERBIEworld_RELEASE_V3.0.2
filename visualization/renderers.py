...
def render_creature(ax, creature: 'Creature', selected: bool = False,
    ...
    # === GATOR-SPECIFIC RENDERING ===
    if creature.species.name == 'Gator':
        _render_gator(ax, creature, selected, light_level, show_label, ec, alpha, lw)
        return
    
    # Main body circle (standard creatures)
    ax.add_patch(Circle(
...
def _render_gator(ax, gator, selected: bool, light_level: float, 
                  show_label: bool, ec, alpha, lw):
    """
    Render Gator with distinctive crocodilian appearance.
    
    Gators have:
    - Elongated body (ellipse instead of circle)
    - Visible tail with oscillation
    - Armored ridge pattern
    - Different water/land coloring
    """
    from matplotlib.patches import Ellipse, Polygon, Wedge
    
    pos = gator.pos
    vel = gator.display_vel if hasattr(gator, 'display_vel') else gator.vel
    
    # Heading from velocity (or default right-facing)
    if np.linalg.norm(vel) > 0.1:
        heading = np.arctan2(vel[1], vel[0])
    else:
        heading = getattr(gator, '_last_heading', 0)
    gator._last_heading = heading
    
    # Colors based on water state
    in_water = getattr(gator, 'in_water', False)
    if in_water:
        body_color = '#2d5a3d'  # Dark aquatic green
        belly_color = '#4a7c59'  # Lighter underbelly
        ridge_color = '#1a3d2a'  # Dark ridges
    else:
        body_color = '#4a5d23'  # Olive/brown on land
        belly_color = '#6b7d3a'
        ridge_color = '#2a3d13'
    
    # Body dimensions
    body_len = gator.species.body_scale * 4.0  # Elongated
    body_width = gator.species.body_scale * 2.0
    
    # === TAIL ===
    tail_phase = getattr(gator, 'tail_phase', 0)
    tail_amplitude = getattr(gator, 'tail_amplitude', 0.3)
    tail_sway = np.sin(tail_phase) * tail_amplitude * 0.5
    
    tail_len = body_len * 0.8
    tail_angle = heading + np.pi + tail_sway  # Behind body with sway
    
    # Tail as tapered polygon
    tail_base = pos - np.array([np.cos(heading), np.sin(heading)]) * body_len * 0.3
    tail_tip = tail_base + np.array([np.cos(tail_angle), np.sin(tail_angle)]) * tail_len
    
    # Perpendicular for tail width
    perp = np.array([-np.sin(tail_angle), np.cos(tail_angle)])
    tail_width_base = body_width * 0.4
    tail_width_tip = body_width * 0.1
    
    tail_points = [
        tail_base + perp * tail_width_base,
        tail_base - perp * tail_width_base,
        tail_tip - perp * tail_width_tip,
        tail_tip + perp * tail_width_tip,
    ]
    ax.add_patch(Polygon(tail_points, fc=body_color, ec=ridge_color, 
                         alpha=alpha * light_level, lw=1))
    
    # Tail ridges
    for i in range(3):
        t = 0.2 + i * 0.25
        ridge_pos = tail_base + (tail_tip - tail_base) * t
        ridge_size = tail_width_base * (1 - t * 0.6)
        ax.add_patch(Circle(ridge_pos, ridge_size * 0.3, 
                           fc=ridge_color, ec='none', alpha=alpha * 0.7 * light_level))
    
    # === MAIN BODY (ellipse) ===
    ellipse = Ellipse(pos, body_len, body_width, angle=np.degrees(heading),
                      fc=body_color, ec=ec, alpha=alpha * light_level, lw=lw)
    ax.add_patch(ellipse)
    
    # Belly stripe
    belly_ellipse = Ellipse(pos, body_len * 0.6, body_width * 0.4, 
                            angle=np.degrees(heading),
                            fc=belly_color, ec='none', alpha=alpha * 0.6 * light_level)
    ax.add_patch(belly_ellipse)
    
    # === HEAD ===
    head_offset = np.array([np.cos(heading), np.sin(heading)]) * body_len * 0.4
    head_pos = pos + head_offset
    head_len = body_len * 0.35
    head_width = body_width * 0.6
    
    # Snout (wedge shape)
    head_ellipse = Ellipse(head_pos, head_len, head_width, angle=np.degrees(heading),
                           fc=body_color, ec=ridge_color, alpha=alpha * light_level, lw=1)
    ax.add_patch(head_ellipse)
    
    # Eyes (on sides of head)
    eye_offset_fwd = np.array([np.cos(heading), np.sin(heading)]) * head_len * 0.2
    eye_offset_side = np.array([-np.sin(heading), np.cos(heading)]) * head_width * 0.35
    
    for side in [1, -1]:
        eye_pos = head_pos + eye_offset_fwd + eye_offset_side * side
        # Eye white
        ax.add_patch(Circle(eye_pos, 0.4, fc='#ffff99', ec='none', 
                           alpha=alpha * light_level))
        # Pupil (vertical slit)
        ax.plot([eye_pos[0], eye_pos[0]], 
                [eye_pos[1] - 0.2, eye_pos[1] + 0.2],
                color='black', lw=1.5, alpha=alpha * light_level)
    
    # === ARMOR RIDGES (along spine) ===
    for i in range(4):
        t = -0.2 + i * 0.2
        ridge_pos = pos + np.array([np.cos(heading), np.sin(heading)]) * body_len * t
        ridge_size = 0.4 * (1 - abs(t))
        ax.add_patch(Circle(ridge_pos, ridge_size, 
                           fc=ridge_color, ec='none', alpha=alpha * 0.8 * light_level))
    
    # === LEGS (stubby) ===
    leg_positions = [
        (0.2, 0.5), (0.2, -0.5),   # Front legs
        (-0.2, 0.5), (-0.2, -0.5),  # Back legs
    ]
    for leg_fwd, leg_side in leg_positions:
        leg_base = pos + np.array([np.cos(heading), np.sin(heading)]) * body_len * leg_fwd
        leg_base += np.array([-np.sin(heading), np.cos(heading)]) * body_width * leg_side
        
        # Short stubby leg
        leg_angle = heading + np.pi/2 * np.sign(leg_side) + np.pi/6 * np.sign(leg_fwd)
        leg_tip = leg_base + np.array([np.cos(leg_angle), np.sin(leg_angle)]) * 1.5
        
        ax.plot([leg_base[0], leg_tip[0]], [leg_base[1], leg_tip[1]],
                color=body_color, lw=2.5, alpha=alpha * 0.8 * light_level)
    
    # === HUNTING INDICATOR ===
    if getattr(gator, 'is_hunting', False):
        # Red eyes when hunting
        for side in [1, -1]:
            eye_pos = head_pos + eye_offset_fwd + eye_offset_side * side
            ax.add_patch(Circle(eye_pos, 0.5, fc='red', ec='none', 
                               alpha=0.6 * light_level))
    
    # === WATER RIPPLES (if in water) ===
    if in_water:
        for i in range(2):
            ripple_size = body_len * (0.8 + i * 0.3)
            ax.add_patch(Circle(pos, ripple_size, fc='none', ec='cyan', 
                               alpha=0.2 * light_level, lw=0.5, ls='--'))
    
    # Label
    if show_label:
        label = f'G{gator.generation}'
        if in_water:
            label += '~'  # Water indicator
        ax.text(pos[0], pos[1] + body_width + 1.0, label,
                color='#88ff88', fontsize=7, ha='center', fontweight='bold')
