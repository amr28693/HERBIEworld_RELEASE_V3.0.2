...
class Terrain:
    ...
    def find_water_bodies(self) -> list:
        """
        Find centers of water bodies in the terrain.
        
        Returns:
            List of dicts with 'center' (world pos), 'size' (cell count), 'depth' (avg)
        """
        from scipy import ndimage
        
        # Create water mask
        water_mask = np.zeros((self.resolution, self.resolution), dtype=bool)
        for i in range(self.resolution):
            for j in range(self.resolution):
                water_mask[i, j] = self.terrain_grid[i, j].name == 'water'
        ...
        for label_id in range(1, n_features + 1):
            ...
            if size < 5:  # Skip tiny puddles
                continue
            
            # Find center of mass
            coords = np.where(component)
            center_i = np.mean(coords[0])
            center_j = np.mean(coords[1])
            
            # Convert to world coordinates
            center_world = self.grid_to_world(int(center_i), int(center_j))
            
            # Average depth (how negative is the height)
            depths = self.heightmap[component]
        ...
        water_bodies.sort(key=lambda x: x['size'], reverse=True)
        return water_bodies
    
    def grid_to_world(self, i: int, j: int) -> np.ndarray:
        """Convert grid indices to world position."""
        half = self.world_size / 2
        cell_size = self.world_size / self.resolution
        x = -half + j * cell_size + cell_size / 2
        y = -half + i * cell_size + cell_size / 2
        return np.array([x, y])
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'scars': self.scars.tolist() if hasattr(self, 'scars') else None,
        }
    ...
    def from_dict(cls, data: dict) -> 'Terrain':
        ...
        # Restore scars or initialize fresh
        if data.get('scars') is not None:
            terrain.scars = np.array(data['scars'])
        else:
            terrain.scars = np.zeros((terrain.resolution, terrain.resolution))
        terrain.scar_decay_rate = 0.0001
        terrain.max_scar_depth = 0.15
