...
class MultiWorld:
    ...
    def __init__(self, terrain: Terrain = None, world_size: float = None, seed: int = None, restoring: bool = False):
        """
        Initialize world.
        ...
        Args:
            ...
            restoring: If True, skip spawning objects (will be restored)
        """
        ...
        # Ghost field reference (set by manager for creature-ghost coupling)
        self._ghost_field = None
        
        # Spawn initial objects (skip if restoring)
        if not restoring:
    ...
    def get_object_potential_for_creature(self, creature: 'Creature') -> np.ndarray:
        ...
        Food creates attractive (negative) potential wells that the body field
        wavefunction evolves toward via phase gradients. This is the EM field
        coupling that allows creatures to "sense" food direction.
        
        Ghosts create subtle attractive/repulsive potentials based on their
        coherence - high coherence ghosts attract, dispersing ones repel.
        
        IMPORTANT: Food potential must not overpower skin containment (~7.0)
        or the wavefunction will escape the body boundary.
        """
        V = np.zeros((Ny, Nx))
        
        # Get creature hunger for coupling strength
        hunger = 0.5
        if hasattr(creature, 'metabolism'):
            hunger = creature.metabolism.hunger
        ...
        for i, obj in enumerate(self.objects):
            if obj.alive:
                ...
                if obj.compliance > 0.5:
                    # Attractive - food creates potential well
                    base_strength = obj.compliance * 0.6 * (1 + obj.energy / (obj.max_energy + 1e-6))
                    hunger_boost = 1.0 + 1.5 * hunger
                    attraction_radius = obj.size * (1.0 + 0.5 * hunger)
                    strength = min(base_strength * hunger_boost * 3.0, 5.0)
                    V -= strength * np.exp(-dist**2 / attraction_radius**2) * (1 + learned)
                else:
                    # Repulsive - barrier
                    barrier_dist = dist - obj.size
                    V += (1 - obj.compliance) * 5.0 * np.exp(-np.maximum(0, barrier_dist)**2 / 0.6)
        
        # === GHOST FIELD COUPLING ===
        # Ghosts create subtle EM disturbances that creatures can sense
        # High coherence = attractive (souls seeking company)
        # Low coherence = slightly repulsive (dispersing energy feels wrong)
        if self._ghost_field is not None:
            for ghost in self._ghost_field.ghosts:
                if not ghost.alive:
                    continue
                rel = ghost.pos - creature.pos
                dist = np.sqrt((X - rel[0])**2 + (Y - rel[1])**2)
                
                # Influence based on ghost coherence and distance
                coherence = ghost._compute_coherence()
                influence = ghost.get_influence_at(creature.pos)
                
                if influence > 0.01:
                    # Coherent ghosts attract (lonely souls)
                    # Dispersing ghosts repel slightly (bad vibes)
                    ghost_potential = influence * (coherence - 0.3) * 2.0
                    V -= ghost_potential * np.exp(-dist**2 / 25.0)
    ...
    def process_creature_interactions(self, creature: 'Creature', body_I: np.ndarray,
        ...
        # Debug: track closest food
        closest_food_dist = 999
        closest_food_contact = 0
        ...
        for i, obj in enumerate(self.objects):
            ...
            # Track closest food for debug
            if obj.compliance > 0.5 and dist < closest_food_dist:
                closest_food_dist = dist
                closest_food_contact = obj.contact
    ...
    def update_ecosystem(self, season: 'Season' = None, fertile_bonus_fn=None):
        ...
        # Process nutrients - pass creature positions for EM coupling
        creature_data = []
        if hasattr(self, '_creature_positions'):
            creature_data = self._creature_positions
        
        for nutrient in self.nutrients[:]:
            if nutrient.update(creature_data):
