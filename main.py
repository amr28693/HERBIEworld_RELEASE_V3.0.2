...
Usage:
    ...
    python -m herbie_world --load FILE     # Load saved world from FILE
    python -m herbie_world --new           # Force new world (ignore saves)
...
def main_visual():
    ...
    save_file = WORLD_STATE_FILE
    if CLI_ARGS and CLI_ARGS.load:
        save_file = CLI_ARGS.load
        print(f"[Persistence] Loading from: {save_file}")
    if CLI_ARGS and CLI_ARGS.save_to:
        save_file = CLI_ARGS.save_to
        print(f"[Persistence] Will save to: {save_file}")
    
    try:
        audio.start()
        
        # Try to load existing world (unless --new specified)
        saved_data = None
        if CLI_ARGS and CLI_ARGS.new:
            print("[Init] --new flag: Starting fresh world")
        else:
            load_path = CLI_ARGS.load if (CLI_ARGS and CLI_ARGS.load) else WORLD_STATE_FILE
            saved_data = WorldPersistence.load_world(load_path)
        
        if saved_data is not None:
            print("[Init] Restoring saved world...")
            terrain = WorldPersistence.restore_terrain(saved_data.get('terrain'))
            if terrain is None:
                print("[Init] Could not restore terrain, creating new")
                world_size = LAUNCH_PARAMS['world_size']
                terrain = Terrain(world_size, TERRAIN_RESOLUTION)
            world = MultiWorld(terrain, restoring=True)
            manager = CreatureManager(world, terrain=terrain, restoring=True)
        ...
        # Store save path for F5 quick save
        manager._save_file_path = CLI_ARGS.save_to if (CLI_ARGS and CLI_ARGS.save_to) else WORLD_STATE_FILE
        ...
        if manager is not None:
            ...
            if hasattr(manager, 'ghost_field'):
                ghost_stats = manager.ghost_field.get_statistics()
            world_history().save_report(manager.seasons.step_count, final_counts, ghost_stats)
            narrative_log().flush()
            
            # === SIMULATION OUTRO ===
            # Only show on clean exit (Ctrl+C), not crashes
            if clean_exit:
                try:
                    from .visualization.simulation_outro import run_outro_from_manager
                    run_outro_from_manager(manager)
                except Exception as e:
                    print(f"[Outro] Could not display analytics: {e}")
...
def main_overnight():
    ...
    try:
        ...
        if manager is not None:
            ...
            if hasattr(manager, 'ghost_field'):
                ghost_stats = manager.ghost_field.get_statistics()
            world_history().save_report(step, final_counts, ghost_stats)
            ...
            try:
                ...
                print(f"[Outro] Analytics unavailable: {e}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='HERBIE World - Emergent Artificial Life Simulation',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m herbie_world                    # Start or continue simulation
  python -m herbie_world --new              # Force new world
  python -m herbie_world --load mysave.npz  # Load specific save file
  python -m herbie_world --overnight        # Headless mode for long runs
  python -m herbie_world --em full          # Full Maxwell EM field
  python -m herbie_world --em off           # Disable EM field

EM Field Modes:
  off          - No electromagnetic field (fastest)
  electrostatic - Electric field only, no time evolution (default)
  full         - Full Maxwell equations with E and B fields (slowest)
        """
    )
    ...
    parser.add_argument('--new', action='store_true',
                       help='Start new world (ignore existing save)')
    parser.add_argument('--load', type=str, metavar='FILE',
                       help='Load world from specific save file')
    parser.add_argument('--save-to', type=str, metavar='FILE',
                       help='Save to specific file instead of default')
    parser.add_argument('--em', type=str, choices=['off', 'electrostatic', 'full'],
                       default=None, metavar='MODE',
                       help='EM field mode: off, electrostatic, full')
    parser.add_argument('--em-resolution', type=int, default=32, metavar='N',
                       help='EM field grid resolution (default: 32)')
    
    args = parser.parse_args()
    
    # Set EM mode via environment variable for creature_manager to pick up
    if args.em:
        em_mode_map = {'off': 'OFF', 'electrostatic': 'ELECTROSTATIC', 'full': 'FULL_MAXWELL'}
        os.environ['HERBIE_EM_MODE'] = em_mode_map[args.em]
    os.environ['HERBIE_EM_RESOLUTION'] = str(args.em_resolution)
    
    # Store in global for access by main_visual/main_overnight
    global CLI_ARGS
    CLI_ARGS = args
