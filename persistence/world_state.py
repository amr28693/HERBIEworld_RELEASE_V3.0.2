...
class WorldPersistence:
    ...
    def save_world(manager, filepath: str = None) -> bool:
        ...
        try:
            ...
            # === DAY/NIGHT ===
            daynight_data = manager.daynight.to_dict() if hasattr(manager, 'daynight') else {}
            ...
            # === LEVIATHAN ===
            leviathan_data = {}
            if hasattr(manager, 'leviathan_mgr') and manager.leviathan_mgr:
                leviathan_data = manager.leviathan_mgr.to_dict()
            
            # === EVOLUTION TREE ===
            evolution_data = {}
            if hasattr(manager, 'evo_tree') and manager.evo_tree:
                evolution_data = manager.evo_tree.to_dict()
            ...
            # === ANT COLONY ===
            ant_colony_data = {}
            if hasattr(manager, 'ant_colony') and manager.ant_colony:
                ant_colony_data = manager.ant_colony.to_dict()
            
            # === GHOST FIELD (V2.6.1) ===
            ghost_field_data = {}
            if hasattr(manager, 'ghost_field') and manager.ghost_field:
                ghost_field_data = manager.ghost_field.to_dict()
            
            # === EM FIELD (V2.6.1) ===
            em_field_data = {}
            if hasattr(manager, 'em_field') and manager.em_field:
                em_field_data = manager.em_field.to_dict()
            
            # === STEP COUNT ===
            step_count = getattr(manager, 'step_count', 0)
            if hasattr(manager, 'seasons'):
                step_count = manager.seasons.step_count
            
            # === RNG STATE (for reproducibility) ===
            # np.random.get_state() returns tuple with mixed types - convert to serializable dict
            rng_raw = np.random.get_state()
            rng_state = {
                'name': rng_raw[0],
                'keys': rng_raw[1].tolist(),  # Convert array to list
                'pos': int(rng_raw[2]),
                'has_gauss': int(rng_raw[3]),
                'cached_gauss': float(rng_raw[4]),
            }
            ...
            np.savez_compressed(
                ...
                ant_colony=ant_colony_data,
                ghost_field=ghost_field_data,      # V2.6.1
                em_field=em_field_data,            # V2.6.1
                
                # Metadata
                save_version="unified_v2",         # Updated version
                save_time=time.time(),
            )
    ...
    def load_world(filepath: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = {
                ...
                'rng_state': d['rng_state'].item() if 'rng_state' in d else None,
                ...
                'ant_colony': d['ant_colony'].item() if 'ant_colony' in d else {},
                'ghost_field': d['ghost_field'].item() if 'ghost_field' in d else {},  # V2.6.1
                'em_field': d['em_field'].item() if 'em_field' in d else {},            # V2.6.1
            }
    ...
    @staticmethod
    def restore_terrain(terrain_data: dict) -> Optional['Terrain']:
        """
        Restore terrain from saved data.
        
        Args:
            terrain_data: Dict from load_world()['terrain']
            
        Returns:
            Terrain object or None if data invalid
        """
        if not terrain_data:
            return None
        ...
        try:
            from ..world.terrain import Terrain
            return Terrain.from_dict(terrain_data)
        except Exception as e:
            print(f"[Persistence] Failed to restore terrain: {e}")
            return None
