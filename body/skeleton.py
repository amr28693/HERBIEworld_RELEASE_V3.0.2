...
class PiezoSkeleton:
    ...
    def step(self, body_I: np.ndarray):
        ...
        Args:
            body_I: Current body intensity field ||^2
        """
    ...
    def inject_vibration(self, frequency: float, amplitude: float):
        """
        Inject external vibration into skeleton (piezoelectric coupling).
        
        Used for coupling to global resonance field - external EM oscillations
        create mechanical stress in piezoelectric skeleton.
        
        Args:
            frequency: Vibration frequency (affects spatial pattern)
            amplitude: Vibration strength
        """
        # Create spatial vibration pattern
        # Higher frequency = finer spatial structure
        x = np.linspace(0, 2*np.pi * max(1, frequency * 10), Nx)
        y = np.linspace(0, 2*np.pi * max(1, frequency * 10), Ny)
        X, Y = np.meshgrid(x, y)
        
        # Standing wave pattern
        pattern = np.sin(X) * np.sin(Y) * amplitude
