...
class BodyField:
    ...
    def evolve(self, V_total: np.ndarray, torus_phase: float, torus_arousal: float, 
        ...
        Args:
            ...
            hunger: Current hunger level (0-1) - modulates potential coupling
        """
        # Update nonlinearity
        g_base = 0.3
        ...
        # Scale potential - HUNGER BOOSTS SENSITIVITY TO FOOD GRADIENTS
        # This is the EM field coupling: hungry creatures are more sensitive
        # to external potential gradients (food "smell")
        # BUT: must not overpower skin containment or wavefunction escapes body
        base_coupling = 0.4
        hunger_coupling = 0.4 * hunger  # Up to 0.4 additional (was 0.8)
        V_scaled = V_total * (base_coupling + hunger_coupling)  # Max 0.8x (was 1.2x)
        ...
        I = np.abs(self.psi)**2
        I_clamped = np.clip(I, 0, 10.0)  # Prevent exponential blowup
