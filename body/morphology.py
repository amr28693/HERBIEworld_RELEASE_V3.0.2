...
class Morphology:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            'skin_stiffness': self.skin_stiffness,
            'skin_thickness': self.skin_thickness,
        }
    
    @classmethod
    def from_dict(cls, d: dict, limb_defs: dict = None) -> 'Morphology':
        """
        Deserialize morphology state.
        
        Args:
            d: Saved state dict
            limb_defs: Species-specific limb definitions (uses LIMB_DEFS if None)
        """
        morph = cls.__new__(cls)
        
        # Use provided limb_defs or fall back to saved angles
        if d.get('limb_angles'):
            morph.limb_angles = d['limb_angles'].copy()
            morph.limb_velocities = d.get('limb_velocities', {name: 0.0 for name in morph.limb_angles})
            morph.limb_torques = d.get('limb_torques', {name: 0.0 for name in morph.limb_angles})
        elif limb_defs:
            morph.limb_angles = {name: info[1] for name, info in limb_defs.items()}
            morph.limb_velocities = {name: 0.0 for name in limb_defs}
            morph.limb_torques = {name: 0.0 for name in limb_defs}
        else:
            # Fall back to default LIMB_DEFS
            morph.limb_angles = {name: info[1] for name, info in LIMB_DEFS.items()}
            morph.limb_velocities = {name: 0.0 for name in LIMB_DEFS}
            morph.limb_torques = {name: 0.0 for name in LIMB_DEFS}
        
        morph.skin_stiffness = d.get('skin_stiffness', 7.0)
        morph.skin_thickness = d.get('skin_thickness', 0.55)
