...
class Metabolism:
    ...
    def update(self, reward: float, mass_extracted: float, creature_vel_mag: float, 
        ...
        # V2.5.3 BALANCE: between too fast (v2.5.1) and too slow (v2.5.2)
        age_factor = 1.0 + 0.4 * (self.age / self.max_age)
        base_hunger_rate = 0.0003 * age_factor * self.metabolism_rate  # Middle ground
        
        # Movement cost: kinetic energy dissipates as metabolic heat
        # Quadratic in velocity (KE ~ v^2), creates exploration/conservation trade-off
        movement_cost = 0.00008 * creature_vel_mag ** 2 * self.metabolism_rate  # Middle ground
        ...
        if reward > 0:
            ...
            # V2.5.3 BALANCE: eating matters but doesn't instantly satisfy
            self.hunger = max(0.0, self.hunger - effective_reward * 0.4 * self.vigor)  # Was 0.8
            self.satiation = min(1.0, self.satiation + effective_reward * 0.3)
            self.last_meal_time = time.time()
        else:
