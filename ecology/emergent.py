...
Systems:
- CultureTracker: Lineage divergence, spatial patterns
...
- TerritorialObserver: Tracks where Herbies spend time (observation only)
...
class SmearableObject:
    ...
    def compute_contact(self, creature_pos: np.ndarray, body_I: np.ndarray) -> float:
        ...
        self.contact = max(proximity_contact, body_contact * 2.0)
        return self.contact
...
class SmearSystem:
    ...
    def _check_smear(self, herbie, terrain, step: int) -> Optional[str]:
        ...
        # Nucleate placement memory - enables emergent return-to-art behavior
        # The soliton evolves via KdV dynamics; whether Herbie returns depends on
        # individual params, hunger level, and soliton persistence
        if hasattr(herbie, 'placement_memory') and hasattr(herbie, 'metabolism'):
            ref_pos = herbie.placement_memory.reference_pos
            drop_direction = np.arctan2(herbie.pos[1] - ref_pos[1], 
                                        herbie.pos[0] - ref_pos[0])
            satiation = 1.0 - herbie.metabolism.hunger
            herbie.placement_memory.nucleate(drop_direction, satiation, object_value=0.5)
...
# =============================================================================
# TERRITORIAL OBSERVER - PURELY watches where Herbies spend time
# =============================================================================

@dataclass
class TerritoryObservation:
    """Observation of where a Herbie spends time. NO behavior modification."""
    center: np.ndarray = field(default_factory=lambda: np.zeros(2))
    time_spent: int = 0
    
    def update(self, pos: np.ndarray, alpha: float = 0.01):
        """Update observed center based on movement."""
        self.center = self.center * (1 - alpha) + pos * alpha
        self.time_spent += 1


class TerritorialObserver:
    """
    PURELY OBSERVATIONAL - watches where Herbies spend time.
    
    This system does NOT:
    - Modify any creature behavior
    - Trigger any attacks or threats
    - Force any movement or decisions
    - Inject any neural signals
    
    It ONLY:
    - Tracks where each Herbie spends time (home range)
    - Documents when Herbies are near each other
    - Reports statistics for analysis
    
    If territorial behavior emerges, it emerges from the creatures'
    own neural dynamics responding to their environment. We just watch.
    """
    
    def __init__(self):
        self.observations: Dict[str, TerritoryObservation] = {}
    
    def update(self, creatures: List, step: int) -> List[str]:
        """Observe. Return nothing - we don't generate events."""
        herbies = [c for c in creatures 
                   if c.species.name == "Herbie" and c.alive]
        
        for herbie in herbies:
            cid = herbie.creature_id
            if cid not in self.observations:
                self.observations[cid] = TerritoryObservation(
                    center=herbie.pos.copy()
                )
            self.observations[cid].update(herbie.pos)
        
        return []  # We don't generate events
    
    def get_home_range(self, creature_id: str) -> Optional[dict]:
        """Get observed home range for a creature."""
        obs = self.observations.get(creature_id)
        if obs is None:
            return None
        return {
    ...
    def get_stats(self) -> dict:
        ...
        return {
            'creatures_observed': len(self.observations),
        }


# Alias for backwards compatibility
TerritorialSystem = TerritorialObserver
