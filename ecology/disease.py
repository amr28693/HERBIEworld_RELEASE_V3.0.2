...
DISEASE_STRAINS = [
    ...
    DiseaseStrain(
        name="Carrion Fever",
        base_virulence=0.12,
        base_lethality=0.006,
        base_duration=300,
        incubation=40,
        contagion_radius=8.0,
        species_targets=['Herbie', 'Blob', 'Mono', 'Biped', 'Scavenger', 'Gator'],
        density_scaling=1.2,
    ),
]

# Special strain for corpse-spawned disease
CARRION_FEVER = DISEASE_STRAINS[-1]
...
class DiseaseSystem:
    ...
    def spawn_from_putrefaction(self, pos: np.ndarray, creatures: List, step: int) -> Optional[str]:
        """
        Spawn a Carrion Fever outbreak from a putrid corpse.
        
        Args:
            pos: Position of the corpse
            creatures: All creatures to potentially infect
            step: Current simulation step
            
        Returns:
            Message if outbreak started, None otherwise
        """
        # Check if we can have another outbreak
        if len(self.active_outbreaks) >= self.max_concurrent_outbreaks:
            return None
        
        # Use Carrion Fever strain
        strain = CARRION_FEVER
        
        # Create outbreak centered on corpse
        outbreak = DiseaseOutbreak(
            disease_id=f"carrion_{step}_{np.random.randint(1000)}",
            strain=strain,
            start_step=step,
            duration_remaining=strain.base_duration,
        )
        
        # Infect nearby creatures immediately
        infected_count = 0
        for creature in creatures:
            if not creature.alive:
                continue
            if creature.species.name not in strain.species_targets:
                continue
            
            dist = np.linalg.norm(creature.pos - pos)
            if dist < strain.contagion_radius:
                # Distance-based infection probability
                infection_prob = strain.base_virulence * (1.0 - dist / strain.contagion_radius)
                if np.random.random() < infection_prob:
                    outbreak.infected_creatures.add(creature.creature_id)
                    outbreak.infection_times[creature.creature_id] = step
                    infected_count += 1
        
        if infected_count > 0:
            self.active_outbreaks.append(outbreak)
            return f"[DISEASE] Carrion Fever spreads from rotting corpse! {infected_count} infected"
