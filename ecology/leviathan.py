...
class LeviathanSystem:
    ...
    def update(self, creatures: List, audio_system=None, world=None) -> dict:
        ...
        # Genesis is now handled by creature_manager, not here
        # (We just track genesis_triggered to avoid double-spawn)
        ...
        # Check spawn conditions (only if no active leviathan and not during genesis)
        if self.leviathan is None and not self.genesis_triggered:
